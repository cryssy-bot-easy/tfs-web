function onBasicDetailsUaLoanMaturityAdjustmentSaveClick(){
	var errors = validateBasicDetailsUaLoanMaturityAdjustmentDmlc();
	onSaveClick(errors);
}
...
function validateBasicDetailsUaLoanMaturityAdjustmentDmlc(){
	if ($("input[name=beneficiaryConformityFlag]").length > 0 && $("input[name=beneficiaryConformityFlag]:checked").val() != 'Y'){
        ...
        triggerAlertMessage("Creation of e-TS is not allowed. Obtain Beneficiary's conformity first. Abort transaction.");
        //triggerAlert();
		return true
	} else {
	var error_msg = ""		
...
function validateDmlcUaLoanMaturityAdjustmentEts(buttonParentId){
