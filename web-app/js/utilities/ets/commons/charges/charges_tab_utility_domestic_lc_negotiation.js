var currency = $("#negotiationCurrency").val();
var settlementCurrency = "PHP";
...
function setupCharges(settlementCurrency) {
    ...
    var currField = "#" + currency + "-USD_special_rate_charges";
    var thirdToUsdSpecialConversionRateCurrency = stripCommas($(currField).val());
    var usdToPhpSpecialConversionRate = getUsdToPhpSpecialConversionRate();

    var productamount = Number($("#negotiationAmount").val());
    if(!productamount){
        productamount="0.00"
    }
    ...
    $.post(recomputeCurrency_LC_DOMESTIC_NEGOTIATION_Url, {
    ...
    $("#REMITTANCECurrency, #remittanceFeePopupFieldCurrency").val(settlementCurrency);
