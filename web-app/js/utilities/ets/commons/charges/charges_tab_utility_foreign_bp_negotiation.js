...
(revision)
SCR/ER Number:
SCR/ER Description: Incorrect Charges Computed (Redmine# 3701)
[Revised by:] Robin C. Rafael
[Date deployed:]
Program [Revision] Details: copied some lines from ebc setup charges function to avoid the error occuring here on EBP
Member Type: JavaScript Pages (js)
Project: WEB
Project Name: charges_tab_utility_foreign_bp_negotiation.js
*/
...
/**
	...
	[Date revised:] 04/25/2018
	Program [Revision] Details: Updated setting value of rate for getThirdToUSDSpecialConversionRate.
...
function getThirdToUSDSpecialConversionRate(settlementCurrency) {
    ...
    if (settlementCurrency == 'PHP') {
        ...
        var rate = $("#" + settlementCurrency + "-USD_special_rate_charges").val();
        if (rate === undefined) {
            rate = $("#" + settlementCurrency + "-USD_special_rate_charges_buying").val();
        }
        return stripCommas(rate);
    }
}
...
function setupCharges(settlementCurrency) {
    ...
    // comment by robs EBP Charges 
    if (thirdToUsdSpecialConversionRateCurrency == undefined) {
    	console.log("ASD if thirds undefined");
    	thirdToUsdSpecialConversionRateCurrency = stripCommas($("#" + currency + "-USD_special_rate_charges_buying").val());
    	console.log("ASD thirdtoUSD: " + thirdToUsdSpecialConversionRateCurrency);
    }
    var usdToPhpSpecialConversionRate = getUsdToPhpSpecialConversionRate();
    var usdToPhpUrr = getUrr("USD");
    if (usdToPhpSpecialConversionRate == undefined) {
    	console.log("ASD of usdtophpspecialconversion undefined");
    	usdToPhpSpecialConversionRate = $("#USD-PHP_special_rate_charges_buying").val();
    	console.log("ASD usdtophpspecialconversionrate: " + usdToPhpSpecialConversionRate);
    }
    //comment end
    ...
    $.post(recomputeCurrency_BP_FOREIGN_NEGOTIATION_Url, {
        ...
        $("#POSTAGE").val(formatCurrency(data.POSTAGE));
        $("#REMITTANCE").val(formatCurrency(data.REMITTANCE));
        $("#CORRES-EXPORT").val(formatCurrency(data['CORRES-EXPORT']));
        ...
        $("#POSTAGEoriginal").val(formatCurrency(data.POSTAGEoriginal));
        $("#REMITTANCEoriginal").val(formatCurrency(data.REMITTANCEoriginal));
        $("#CORRES-EXPORToriginal").val(formatCurrency(data['CORRES-EXPORToriginal']));
    ...
    $("#CORRES-EXPORTCurrency").text(settlementCurrency);
    $("#confirmingFeePopupFieldCurrency").val(settlementCurrency);
    $("#CORRES-EXPORTCurrency").text(settlementCurrency);
    $("#POSTAGECurrency").val(settlementCurrency);
    ...
    $("#POSTAGECurrency").text(settlementCurrency);
...
function updateTotals() {
    ...
    $.post(computeTotalUrl, {amounts:amounts.toString()}, function (data) {
        ...
        if(typeof setupProceedsPayment !== 'undefined'){
