...
function setupCharges(settlementCurrency) {
    ...
    $("#ADVISING-EXPORT").val("");
    if ($("#OTHER-EXPORT").val() != undefined) {
        $("#otherAdvisingFee").val("");
    }
    ...
    $.post(recomputeCurrency_EXPORTS_ADVISING_Url, {
        ...
        $("#ADVISING-EXPORT").val(formatCurrency(data['ADVISING-EXPORT']));

        if (data['OTHER-EXPORT'] != null) {
            $("#OTHER-EXPORT").val(formatCurrency(data['OTHER-EXPORT']));
        }
        if (data.CABLE != null) {
        ...
        $("#ADVISING-EXPORToriginal").val(formatCurrency(data['ADVISING-EXPORToriginal']));
        if (data['OTHER-EXPORToriginal'] != null) {
            $("#OTHER-EXPORToriginal").val(formatCurrency(data['OTHER-EXPORToriginal']));
        }
        if (data.CABLEoriginal != null) {
    ...
    $("#ADVISING-EXPORTCurrency, #exportsAdvisingFeeCurrency, #esportsAdvisingFeePopupFieldCurrency").val(settlementCurrency);
    $("#OTHER-EXPORTCurrency, #otherAdvisingFeeCurrency, #otherAdvisingFeeCurrency").val(settlementCurrency);
    $("#OTHER-EXPORTCurrency, #otherAdvisingFeeCurrency, #otherAdvisingFeeCurrency").text(settlementCurrency);
    $("#otherLocalBankChargesPopupFieldCurrency").text(settlementCurrency);
    $("#otherLocalBankChargesPopupFieldCurrency").val(settlementCurrency);

    if ($("#CABLE").val() != undefined) {
        $("#CABLECurrency, #cableFeeCurrency, #cableFeePopupFieldCurrency").val(settlementCurrency);
    }
}
...
function setupChargesPopup() {
    ...
    if(translatedFieldId == 'ADVISING-EXPORT'){
        var temp = $('#cwtFlag').val();
        if(temp =='Y'|| temp =='Yes'||temp =='1'){
            $("#ADVISING-EXPORT").val('');
            var origAmount = $("#" + activePopupDiv + "Field").val();

            var newAmount = Number(origAmount.replace(',',''));
            var cwtAmount = newAmount * 0.98 ;
            $("#ADVISING-EXPORT").val(formatCurrency(cwtAmount));

            // compute cwt percent
//            $.post(computeCwtUrl, {origAmount:origAmount}, function (data2) {
//
//            });
...
function constructAmountList() {
    ...
    if ($("#ADVISING-EXPORT").val() != undefined) {
        amounts.push($("#ADVISING-EXPORT").val().replace(/,/g, "*"));
    }
    if ($("#OTHER-EXPORT").val() != undefined) {
        amounts.push($("#OTHER-EXPORT").val().replace(/,/g, "*"));
    }
    if ($("#CABLE").val() != undefined) {
...
function setCurrency(settlementCurrency) {
    ...
    if ($("#ADVISING-EXPORTCurrency").length > 0) {
        $("#ADVISING-EXPORTCurrency").text(settlementCurrency);
    }

    if ($("#OTHER-EXPORTCurrency").length > 0) {
        $("#otherAdvisingFeeCurrency").text(settlementCurrency);
    }
