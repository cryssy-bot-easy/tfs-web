/*	PROLOGUE:
	...
	Program [Revision] Details: Added CABLE and DOCSTAMP in function constructAmountList().
	Date deployment: 
	Member Type: JAVASCRIPT
	Project: WEB
	Project Name: charges_tab_utility_foreign_lc_amendment.js
*/
...
function setupCharges(settlementCurrency) {
    ...
    $.post(recomputeCurrency_LC_FOREIGN_AMENDMENT_Url, {
    ...
    $("#CABLECurrency, #cableFeeCurrency").text(settlementCurrency);
    $("#DOCSTAMPSCurrency, #documentaryStampCurrency").text(settlementCurrency);
    $("#CFCurrency, #commitmentFeePopupFieldCurrency, #commitmentFeeNetBankComCurrency, #commitmentFeeCWTCurrency, #commitmentFeeGrossBankComCurrency").val(settlementCurrency);
    $("#BCCurrency, #bankCommissionPopupFieldCurrency, #bankCommissionNetBankComCurrency, #bankCommissionCWTCurrency, #bankCommissionGrossBankComCurrency").val(settlementCurrency);
    $("#CABLECurrency, #cableFeePopupFieldCurrency").val(settlementCurrency);
    $("#DOCSTAMPSCurrency, #documentaryStampPopupFieldCurrency").val(settlementCurrency);
    $("#CORRES-ADVISINGCurrency, #advisingFeePopupFieldCurrency").val(settlementCurrency);
    $("#CORRES-CONFIRMINGCurrency, #confirmingFeePopupFieldCurrency").val(settlementCurrency);
...
function onChangeCorresCharges() {
    ...
    if (advanceCorresCharges == "Y") {
        $(".corres_hide").show();
        $("#CORRES-ADVISING, #advisingFeePopupParams, #confirmingFee, #confirmingFeePopupParams").attr("disabled", false);
    } else {
        $(".corres_hide").hide();
        $("#CORRES-ADVISING, #advisingFeePopupParams, #confirmingFee, #confirmingFeePopupParams").attr("disabled", "disabled");
    }
}
