...
function setupCharges(settlementCurrency) {
    ...
    $.post(recomputeCurrency_IMPORT_ADVANCE_PAYMENT_Url, {
...
function initializeChargesTabUtility() {
    ...
    $("#settlementCurrency").change(function() {
    	//loadForeignExchangeRates(); // removed this since it reloads the rates table which should not be happening
    	onSettlementCurrencyChange();
    });

    $("#settlementCurrency").setCurrencyDropdown($(this).attr("id"));
    //hideForeignExchangeRates();
