...
function setupCharges(settlementCurrency) {
    ...
    $.post(recomputeCurrency_LC_FOREIGN_INDEMNITY_CANCELLATION_Url, {
        tradeServiceId: tradeServiceId,
        cwtFlag: cwtflag
    }, function (data) {
        $("#CANCEL").val(formatCurrency(data.CANCEL));
        $("#CANCELoriginal").val(formatCurrency(data.CANCELoriginal));
...
//    $("#documentaryStampPopupFieldCurrency").val(settlementCurrency);
    $("#CANCELCurrency").text(settlementCurrency);
...
function resetCurrencyField() {
    ...
    if ($("#DOCSTAMPSCurrency").length > 0) {
        $("#DOCSTAMPSCurrency").text("");
    }

    if ($("#totalAmountChargesCurrency").length > 0) {
        $("#totalAmountChargesCurrency").text("");
    }
