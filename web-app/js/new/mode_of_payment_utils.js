 ...
 * SCR/ER Number:
 * SCR/ER Description: 
 * 1. (Redmine #4144) In settlement tab, Mode of Payment-Credit to CASA; default should be CIF instead of OTHERS. 
 * 2. (Redmine 4216) In Negotiation tab: Amount of Negotiation Payment(in Settlement Currency) and 
 *  Amount of Negotiation Payment(in Negotiation Currency) should still be displayed after saving.
 * [Revised by:] Ludovico Anton Apilado
 * [Date revised:] 
 * 1. (for redmine 4144) 2/14/2017
 * 2. (for redmine 4216) 5/3/2017
 * Program [Revision] Details: 
 * 1. (for redmine 4144) on changeModeOfPayment() function, revised condition in: if modeOfPaymentCharges = "credit to casa" 
 *  - mode of payment option is casa only (option: others before revision) Show select dropdown.
 * 2. (for redmine 4216) on insertToNonMd() function, condition if (validationResult == "true") 
 * 	commented line that makes amount of nego null after saving. 
 * Date deployment: 6/16/2017
 * Member Type: JS
 * Project: WEB
 * Project Name: mode_of_payment_utils.js
 */

/**
 * (revision)
 *	SCR/ER Number: 
 *	SCR/ER Description: In EBP Nego, no prompt message appeared if more than one mode of Settlement Charges 
 *		   				selected in payment of Charges (Redmine# 4156)
 *	[Revised by:] Brian Harold A. Aquino
 *	[Date revised:] 02/08/2017 (tfs-web Rev# 7357), 02/14/2017 (tfs-web Rev# 7284)
 *	[Date deployed:] 06/16/2017
 *	Program [Revision] Details:  Charges Payment tab was added in the condition for validating the number of mode of settlement.
 *	Member Type: JS
 *	Project: WEB
 *	Project Name: mode_of_payment.js
 */

function triggerAlertMessageUtils(alertMessage){
    $("#alertMessage").text(alertMessage);
    triggerAlert();
}


function closeAlertModeOfPayment() {
...
function openAlertModeOfPayment() {
    var mSave_div = $("#popup_save_confirmation");
    var mBg = $("#confirmation_background");
...
//            alert($("#" + $("#currency").val() + "-USD_special_rate_cash").val());
...
function constructIbTrLoanForm() {
    ...
    var loanAmount = $("#cashAmountInSettlement").val();
    if (documentClass == 'BP') {
    ...
    var loanPaymentCode = $("#loanPaymentCode").val();
    var withCramApproval = $("input[name=cramApprovalFlag]:checked").val();
    var numberOfFreeFloatDays = 0;
    if($("#numberOfFreeFloatDaysNegotiation").val()){
    	numberOfFreeFloatDays = $("#numberOfFreeFloatDaysNegotiation").val();
    }

    var setupStringList = new Array();

    setupStringList.push("bookingCurrency=" + bookingCurrencyIbTr);
    setupStringList.push("loanAmount=" + loanAmount);
//    setupStringList.push("interestRate=" + interestRate);
    ...
    setupStringList.push("loanPaymentCode=" + loanPaymentCode);
    setupStringList.push("withCramApproval=" + withCramApproval);
    setupStringList.push("numberOfFreeFloatDays=" + numberOfFreeFloatDays);
...
function getPopupFieldParameters(typeOfLoan) {
    //alert(typeOfLoan);
    if(typeOfLoan == "IB_LOAN" || typeOfLoan == "TR_LOAN" || typeOfLoan == "DBP" || typeOfLoan == "EBP") {
...
function onDeletePayment(id){
	if(formId == "#proceedsDetailsTabForm" && $("#grid_list_charges_payment").length > 0) {
		...
		var postUrl = _validateSavedChargesPaymentsUrl;
...
function continueDeletePayment(id) {
    ...
    if(formId.indexOf("cash") == -1 && formId != "#productPaymentTabForm"){
    	...
    	if(documentClass in {"IMPORT_ADVANCE":1, "EXPORT_ADVANCE":1} && serviceType == 'REFUND'){
    		gridId = $("#grid_list_proceeds_payment_summary");
    	} else {
    		gridId = $("#grid_list_cash_payment_summary");
    	}
    ...
    if(status in {"Not Paid":1, "Rejected":1} ||
        (status == "Paid" && modeOfPayment in {"PDDTS":1, "SWIFT":1, "MC_ISSUANCE":1})
        ) {
...
//                        alert('i am cdt');
                        if (clearPaymentReferenceByMC) {
                            $.post(clearPaymentReferenceByMC, {}, function(data) {
            ...
            }).done(validatePaymentStatusForPrepareButton);
        } else {
...
function validatePaymentStatusForPrepareButton(){
	if (documentClass == "MD" ||
			(documentClass == "IMPORT_CHARGES" && serviceType != "REFUND") ||
			(documentClass in {"IMPORT_ADVANCE":1, "EXPORT_ADVANCE":1, BP:1, CORRES_CHARGE:1, BC:1} && referenceType == "DATA_ENTRY")) {
...
function dispatchCommand(id) {
    ...
    if((referenceType == "DATA_ENTRY" || referenceType == "PAYMENT") && formId == "#chargesPaymentTabForm"){
        ...
        $.post(postUrl, params, function(data) {
            $("#grid_list_charges_payment").jqGrid('setGridParam', {datatype: 'json', url: serviceChargeUrl, page: 1}).trigger("reloadGrid");
        });

    } else if((referenceType == "DATA_ENTRY" && documentClass == "BP" && serviceType == "SETTLEMENT") || 
    		(referenceType == "DATA_ENTRY" && formId == "#cashLcPaymentTabForm" && documentClass == "BC" && serviceType == "SETTLEMENT") || 
    		(referenceType == "DATA_ENTRY" && formId == "#productPaymentTabForm") || 
    		(referenceType == "PAYMENT" && (formId == "#cashLcPaymentTabForm" || formId == "#productPaymentTabForm")) ||
    		(formId == "#paymentDetailsTabForm" && referenceType == "DATA_ENTRY") ||
    		(referenceType == "DATA_ENTRY" && formId == "#cashLcPaymentTabForm" && documentClass == "CORRES_CHARGE" && serviceType == "SETTLEMENT")) {

        var paymentsummarydata = $("#grid_list_cash_payment_summary").jqGrid("getRowData", id);

        if (documentClass == "CDT") {
            paymentsummarydata = $("#grid_list_payment_cdt").jqGrid("getRowData", id);
        }
        ...
        if (documentClass == "BP" && serviceType == "SETTLEMENT" && referenceType == "DATA_ENTRY" && documentType == "FOREIGN") {
            params["passOnRateConfirmedByCash"] = $("#passOnRateConfirmedByCash").val();
        }

        $.post(postUrl, params, function(data) {
            if(data.success == "false") {
                $("#grid_list_cash_payment").jqGrid('delRowData',id);
            }
            $("#grid_list_cash_payment_summary").jqGrid('setGridParam', {datatype: 'json', url: productChargeUrl, page: 1}).trigger("reloadGrid");
        });
    ...
    } else if ((referenceType == "DATA_ENTRY" || referenceType == "PAYMENT") && (formId == "#proceedsDetailsTabForm" || (formId == "#cashLcPaymentTabForm" && serviceType.toUpperCase() == 'REFUND'))) {
        var paymentsummarydata = $("#grid_list_proceeds_payment_summary").jqGrid("getRowData", id);
        ...
        $.post(postUrl, params, function(data) {
            ...
            if (settlementToBenGridUrl) {
                var gridurl = settlementToBenGridUrl + "?tradeServiceId="+$("#tradeServiceId").val();
                $("#grid_list_proceeds_payment_summary").jqGrid('setGridParam', {datatype: 'json', url: gridurl, page: 1}).trigger("reloadGrid");
                $("#popup_btn_mode_of_payment_proceeds").removeAttr("disabled");
            }
            if ('function' === typeof setupProceedsPayment)
            setupProceedsPayment();
        }).done(validatePaymentStatusForPrepareButton);
    } else if (referenceType == "PAYMENT" && serviceType == "PAYMENT" && documentClass == "CDT") {
        ...
        $.post(postUrl, params, function(data) {
            ...
            } else { // do this to get actual payment detail id after saving
                $("#grid_list_payment_cdt").jqGrid('setGridParam', {datatype: 'json', url: productChargeUrl, page: 1}).trigger("reloadGrid");
                if (getPaymentReferenceByMCUrl) {
                    $.post(getPaymentReferenceByMCUrl, {ied: $("#iedieirdNumber").val()}, function(data) {
...
function dispatchDeleteCommand(id) {
    ...
    if(((referenceType == "PAYMENT" || referenceType == "DATA_ENTRY") && formId == "#chargesPaymentTabForm")) {
        var paymentsummarydata = $("#grid_list_charges_payment").jqGrid("getRowData", id);
    ...
    } else if((referenceType == "DATA_ENTRY" && formId == "#productPaymentTabForm") ||
    	(referenceType == "DATA_ENTRY" && formId == "#cashLcPaymentTabForm" && serviceType.toUpperCase() != 'REFUND') ||
        (referenceType == "PAYMENT" && (formId == "#cashLcPaymentTabForm" || formId == "#productPaymentTabForm")) || 
        (formId == "#paymentDetailsTabForm" && referenceType == "DATA_ENTRY")) {
        var paymentsummarydata = $("#grid_list_cash_payment_summary").jqGrid("getRowData", id);
    ...
    } else if((referenceType == "PAYMENT" || referenceType == "DATA_ENTRY") && (formId == "#proceedsDetailsTabForm" || (formId == "#cashLcPaymentTabForm" && serviceType.toUpperCase() == 'REFUND'))) {
        var paymentsummarydata = $("#grid_list_proceeds_payment_summary").jqGrid("getRowData", id);
...
function insertToProceeds() {
    ...
    if(referenceType == "ETS" || (referenceType == "DATA_ENTRY" && !(documentClass in {BC: 1,BP: 1}))) {
    if(documentClass in {'IMPORT_ADVANCE':1, 'EXPORT_ADVANCE':1} && serviceType == 'REFUND'){
    	var rates = constructRates('cash');
    	addData = {
			modeOfPayment: modeOfPaymentText,
            settlementCurrency: $("#refundCurrency").val(),
            amountSettlement: $("#importAdvanceAmount").val(),
            amountInRefundCurrency: $("#amountInDefCurrency").val(),
			deletePaymentSummary: deletePaymentSummary,
			paymentMode: modeOfPayment,
			rates: rates,
			pay: payString,
        ...
        var currency = $("#amountOfPaymentCurrencyProceedBeneficiary").val();
        var amount = $("#amountOfPaymentProceedBeneficiary").val();

        if (currency == undefined) {
        	if($("#proceedsCurrencyModified").length > 0){
        		currency = $("#proceedsCurrencyModified").val();
        	} else {
        ...
        if (amount == undefined) {
        	if($("#proceedsAmountModified").length > 0){
        		amount = $("#proceedsAmountModified").val();
        	} else {
        		amount = $("#proceedsAmount").val();
        	}
        }

        addData = {
            modeOfPayment: modeOfPaymentText,
            settlementCurrency: currency, //$("#amountOfPaymentCurrencyProceedBeneficiary").val(),
            amountSettlement: amount, //$("#amountOfPaymentProceedBeneficiary").val(),
            deletePaymentSummary: deletePaymentSummary,
            paymentMode: modeOfPayment,
            tradeSuspenseAccount: tradeSuspenseAccount,
            accountNumber: accountNumber,
    ...
    } else if (referenceType == "PAYMENT" || (referenceType == "DATA_ENTRY" && (documentClass in {BC: 1,BP: 1}))) {
        var currency = $("#amountOfPaymentCurrencyProceedBeneficiary").val();
        var amount = $("#amountOfPaymentProceedBeneficiary").val();
    ...
    if(validateAp == "success") {
    	if(documentClass in {'IMPORT_ADVANCE':1, 'EXPORT_ADVANCE':1} && serviceType == 'REFUND'){
    		if((($("#totalRefundAmount").val() ? parseFloat($("#totalRefundAmount").val().replace(",","")) : 0.00) + parseFloat($("#importAdvanceAmount").val().replace(",",""))) > parseFloat($("#importAdvanceDueAmount").val().replace(",",""))) {
    			if(documentClass == 'IMPORT_ADVANCE'){
    				triggerAlertMessage("Amount of Refund cannot be greater than Import Advance Amount.")
    			} else {
    				triggerAlertMessage("Amount of Refund cannot be greater than Export Advance Amount.")
    			}
    			return;
    		}
    	}
        ...
        if(validationResult == "true") {
            ...
            if (referenceType == "PAYMENT" || referenceType == "DATA_ENTRY") {
...
// insert to charges payment grid
    ...
    if (formId.indexOf("cash") != -1 || formId == "#productPaymentTabForm") {
        ...
        if (documentClass != "IMPORT_CHARGES" && documentClass != "EXPORT_CHARGES") {
    ...
    if (modeOfPayment == "CASA") {
        ...
        payString = "<input type=\"button\" class=\"input_button\" value=\"Pay\" onclick=\"var id=\'" + id + "\'; showLoanDetails(id);\"/>";
    } else if (modeOfPayment == "EBP") { // specifically for negotiation
    	setupString = getPopupFieldParameters("EBP");
    	payString = "<input type=\"button\" class=\"input_button\" value=\"Pay\" onclick=\"var id=\'" + id + "\'; showLoanDetails(id);\"/>";
    }
    ...
    if (formId.indexOf("cash") == -1 && formId != "#productPaymentTabForm") {
        if (referenceType == "ETS") {
            ...
            if (documentClass in {EXPORT_CHARGES:1, CORRES_CHARGE:1}) {
                addData = {
                    accountNumber: accountNumber,
                    modeOfPayment: modeOfPaymentText,
                    settlementCurrency: "PHP", //$("#settlementCurrency").val(),
                    amount: tempAmountOfPaymentCharges,
                    amountSettlement: tempAmountOfPaymentCharges,
                    deletePaymentSummary: deletePaymentSummary,
                    status: "Not Paid",
            ...
            } else if (documentClass == "BC" || documentClass == "BP") {
                amountSettlement = tempAmountOfPaymentCharges;
            ...
            } else if (documentClass == "IMPORT_ADVANCE" || documentClass == "EXPORT_ADVANCE") {
                amountSettlement = $("#amountOfPaymentCharges").val();
                
                addData = {
                    accountNumber: accountNumber,
                    modeOfPayment: modeOfPaymentText,
                    settlementCurrency: $("#amountOfPaymentChargesSettlementCurrency").text(),
                    amountSettlement: $("#amountOfPaymentCharges").val(),
                    deletePaymentSummary: deletePaymentSummary,
                    status: "Not Paid",
                    pay: payString,
            ...
            } else if (documentClass == "IMPORT_CHARGES") {
...
//            if ((documentClass == "BP" && documentType == "FOREIGN" && serviceType == "SETTLEMENT" && referenceType == "DATA_ENTRY")) {
//
//                payString = "";
//            }
                addData = {
...
//            }
            ...
            //commented for fix of bug 4216 - Vico
//            $("#cashAmountInSettlement, #cashAmountInLc").val("").removeAttr("disabled").removeAttr("readonly");
    ...
    if (referenceType == "DATA_ENTRY" && formId == "#productPaymentTabForm" && documentClass != "CORRES_CHARGE" && (documentClass != 'BP' && serviceType != 'SETTLEMENT')) {
        dispatchCommand(id);
    }
}
...
function insertToChargesPaymentGrid() {
    if(formId == "#paymentDetailsTabForm") {	
        insertToMd();
    } else if(formId == "#proceedsDetailsTabForm" || (formId == "#cashLcPaymentTabForm" && documentClass in {'IMPORT_ADVANCE':1, 'EXPORT_ADVANCE':1}) && serviceType == 'REFUND') {
        insertToProceeds();
    } else {
...
function insertToCdt() {
    ...
    var amountToAdd = parseFloat(tempAmountOfPaymentCharges.replace(/,/g, ""));
    var totalAmountAdded = parseFloat($("#totalAmountOfPayment").val().replace(/,/g, ""));
    var totalAmountDue = parseFloat($("#totalAmountDue").val().replace(/,/g, ""));

    var totalAmountPaid = amountToAdd + totalAmountAdded

    if ((totalAmountPaid > totalAmountDue) && modeOfPayment != "REMITTANCE") {
        triggerAlertMessage("Excess not allowed using this mode of payment.");
    } else {
        var addData;
        ...
        var validateCdt = validateCdtBankCharge($("#defaultBankCharge").val(), $("#cdtBankChargeDefaultValue").val());
        
        if(validateCdt == "success"){
        	var validationResult = validateInsertValues(gridToInsert, modeOfPayment);
...
function validateCdtBankCharge(newBankCharge, defaultBankCharge) {
	newBankCharge = parseFloat(newBankCharge.replace(/\,/g, ""));
	defaultBankCharge = parseFloat(defaultBankCharge.replace(/\,/g, ""));

    if (defaultBankCharge != newBankCharge) {
        $("#alertMessage").text("Cannot add mode of payment. Please input Bank Change first and save..");
        triggerAlert();
        return "fail";
    }
...
function updateTotalsCdt() {
    ...
    $.post(computeTotalUrl,{amounts: amounts.toString()},function(data) {
        $("#totalAmountOfPayment").val(formatCurrency(data.totalAmount)).change();

        var due = parseFloat($("#totalAmountDue").val().replace(/,/g, ""));
        var totalAmountPaid = parseFloat(data.totalAmount.replace(/,/g, ""));
        var balance = due - totalAmountPaid;
        $("#remainingCDTBalance").val(formatCurrency(balance));
//        $("#amountOfPayment").val($("#totalAmountDue").val());

        $("#amountOfPayment").val("");

        if (balance > 0) {
            // $("#amountOfPayment").val($("#totalAmountDue").val());
            $("#amountOfPayment").val(formatCurrency(balance));
        }
    });
}
...
//    	var due = parseFloat($("#proceedsAmount").val().replace(/,/g,""));
        var due = $('[name="amountForCredit"]').val() ? parseFloat($('[name="amountForCredit"]').val().replace(/,/g,"")) : parseFloat($('[name="proceedsAmount"]').val().replace(/,/g,""));
...
function updateTotalAmountOfPaymentCharges(form) {
    if ((documentClass == "LC" && documentType == "DOMESTIC" && serviceType.toUpperCase() == "NEGOTIATION" && form == "#proceedsDetailsTabForm") || form == "#proceedsDetailsTabForm") {
        ...
        if((form.indexOf("cash") == -1 && form != "#productPaymentTabForm")  && form.indexOf("negotiationPayment") == -1) {
            ...
            if(form == "#basicDetailsTabForm" && documentClass == "AP") {
                if(referenceType == "ETS") {
            ...
            } else if(form == "#basicDetailsTabForm" && documentClass == "AR") {
                if(referenceType == "ETS") {
                    gridData = $("#grid_list_refund_branch").jqGrid("getRowData");
                } else {
                    gridData = $("#grid_list_refund_main").jqGrid("getRowData");
                ...
                balance = $("#remainingBalance, #amountOfPaymentCharges");
                ...
                $.each(gridData,function(idx, data) {
                    if(referenceType == "ETS") {
                        ...
                        amounts.push(data.amountSettlement.replace(/,/g,"*"));
                    } else if (referenceType == "DATA_ENTRY" && (serviceType == "PAYMENT" || serviceType == "PAYMENT_OTHER") && (documentClass == "IMPORT_CHARGES" || documentClass == "IMPORT_ADVANCE" || documentClass == "EXPORT_CHARGES" || documentClass == "EXPORT_ADVANCE")) {
                        amounts.push(data.amountSettlement.replace(/,/g,"*"));
                    } else if (referenceType == "DATA_ENTRY" && serviceType == "REFUND" && documentClass in {"IMPORT_ADVANCE":1, "EXPORT_ADVANCE":1}) {
                    	amounts.push(data.amountSettlement.replace(/,/g,"*"));
                    } else if (referenceType == "DATA_ENTRY" && documentClass == "BP") {
                        amounts.push(data.amountSettlement.replace(/,/g,"*"));
                    } else if (referenceType == "DATA_ENTRY" && serviceType == "SETTLEMENT" && documentClass == "BC") {
                        amounts.push(data.amountSettlement.replace(/,/g,"*"));
                    }
                })
            }
        	...
        	if(serviceType == "REFUND"){
        		if(documentClass in {"IMPORT_ADVANCE":1, "EXPORT_ADVANCE":1}){
        			gridData = $("#grid_list_proceeds_payment_summary").jqGrid("getRowData");
        			due = $("#importAdvanceDueAmount");
        			balance = $("#importAdvanceAmount");
	...
	//            if ($("#netBillingAmount").length > 0) {
	//                due = $("#netBillingAmount");
	//            }
	
	            if ($("#netBillingAmountInBillingCurrency").length > 0) {
	                due = $("#netBillingAmountInBillingCurrency");
	            }
            ...
            $.each(gridData,function(idx, data) {
                ...
                if ((((serviceType == "UA Loan Settlement" || serviceType == "UA_LOAN_SETTLEMENT") &&
                     ...
                     (referenceType == "ETS" || referenceType == "DATA_ENTRY"))) ||
                    documentClass == "CORRES_CHARGE" ||
                    (documentClass == "IMPORT_CHARGES" && serviceType == "PAYMENT")) {
                    amounts.push(data.amountSettlement.replace(/,/g,"*"));
                } else {
                    //alert(data.amount)
...
//    alert(computeTotalUrl)

        if(computeTotalUrl != "") {
            // compute total payment charges
            //alert(amounts.toString())
            var allAmount = 0;
            if (amounts.toString() != "") {
...
//                alert(computedTotalAmount)
                total.val(computedTotalAmount.substr(0,computedTotalAmount.length - 2));

                if (documentClass == "CORRES_CHARGE" && referenceType == "DATA_ENTRY" && serviceType.toUpperCase() == "SETTLEMENT") {
                    if ($("#netAmountMt").length > 0) {
                        $("#netAmountMt").val(computedTotalAmount.substr(0,computedTotalAmount.length - 2));
                    }
                }
                ...
                if ($("#proceedsDetailsTabForm").length > 0) { // if proceeds is available
                    if (documentSubType1 != null && documentSubType1.toUpperCase() == "CASH") {
                        ...
                        if ($("#productAmount").length > 0 && (documentClass == 'DR' ||
                            ...
                            documentClass == 'DP' ||
                            documentClass == 'OA')) {
                            $("#proceedsAmount").val($("#productAmount").val());
                        }
...
//                        }
                        ...
                        if(documentSubType1 && documentSubType1 == "CASH") {
                            disableCashLc();
        ...
        if(formId == "#cashLcPaymentTabForm" || formId == "#productPaymentTabForm") {	// displays all related currencies for cash lc grid
...
function validateGridValues() {
    ...
    if(rowCount == 1 && formId == "#proceedsDetailsTabForm") {
        ...
        success = false;
    } else if(rowCount == 1 && '#chargesPaymentTab') {
    	$("#alertMessage").text("Only one mode of payment is allowed.");
        success = false;
    } else if(rowCount == 3) {
...
function onConstructModesOfPayment () {
    ...
    var settlementCurrency = $("#settlementCurrency").val();
    if (formId == "#proceedsDetailsTabForm") {
    	if($("#newProceedsCurrency").length > 0){
    		if(!$("#newProceedsCurrency").val()){
    			triggerAlertMessage("Proceeds Currency is required.");
    			return false;
    		} else {
    			settlementCurrency = $("#proceedsCurrencyModified").val();
    		}
    	} else {
    		settlementCurrency = $("#amountOfPaymentCurrencyProceedBeneficiary").val();
    	}
        ...
        if($("#newRefundCurrency").length > 0){
        	if(!$("#newRefundCurrency").val()){
    			triggerAlertMessage("Refund Currency is required.");
    			return false;
    		} else {
    			settlementCurrency = $("#refundCurrency").val();
    		}
    ...
    var params = {
        ...
        exportViaPddtsFlag: $("input[name=exportViaPddtsFlag]:checked").val()
    }
...
//	parent_id = this.id;
    ...
    $(".display-ib-tr-loan :input[id=numberOfFreeFloatDaysNegotiation]").val("0");
    ...
    if(thisid.indexOf("cash") != -1) {
        if ($("#serviceType").val().toLowerCase() != "negotiation" && ($("#serviceType").val().toLowerCase() != "ua loan settlement" || $("#serviceType").val().toLowerCase() != "ua_loan_settlement")){
            $('#popup_header_modeOfPaymentCharges').text("Mode of Payment - Cash LC");
        ...
        $('#popup_header_modeOfPaymentCharges').text("Mode of Payment - Settlement to Beneficiary");
    }else{
        $('#popup_header_modeOfPaymentCharges').text("Mode of Payment - Charges Payment");
    }
...
function changeModeOfPayment() {
    $("#apReferenceId").val("");
//    $("#arReferenceId").val("");
//    alert($("#modeOfPaymentCharges").val() + " : " + $("#modeOfPaymentCharges option[value='"+$("#modeOfPaymentCharges").val()+"']").text());
...
//	var dNCharges2 = $("#documentReferenceNumberAr")
    ...
    var apBalDisplay2Charges=$(".display-ap-balance2_charges");
    var apBalDisplay1Charges=$(".display-ap-balance1_charges");
    var ibDbpTrDtr=$('.display-ib-tr-loan').css("display","none");
    var uaDua=$('.display-ua-loan').css("display","none");
    $(".display_ibt_branch").css("display","none");
...
//        dNCharges2.val("");
        ...
        $("#modeCheck").val($("#accountNumberCheck").val());
        ...
        if(typeof documentClass !== 'undefined' && documentClass == "CDT"){
        	$("#accountNumberCheck").val("O");
//        	$("#accountNumber").select2(data,{id:$("#casaAccountNumber").val()});
        	$("#accountNumber").toggleClass("select2_dropdown", false).toggleClass("input_field", true).val("").select2('destroy');
        	$("#accountNameCheck").show();
        	$("#accountNumber").val($("#casaAccountNumberCdt").val());
...
//        dNCharges2.val("");
        ...
        //changed mode of payment of credit to casa:
        //from Others to CIF
        //@author: Ludovico Anton Apilado
        $("#accountNumberCheck").hide();
        $("#accountNameCheck").hide();
        $("#accountNumber").toggleClass("select2_dropdown", true).toggleClass("input_field", false).val("").queryCasaAccountNumbersByCurrency((formId == "#cashLcPaymentTabForm" || formId == "#productPaymentTabForm" || formId == "#paymentDetailsTabForm" || formId =="#proceedsDetailsTabForm") ? "#newProceedsCurrency" : "#settlementCurrency");
        centerPopup("popup_modeOfPaymentCharges", "mode_of_payment_charges_bg");
...
//	}else if(pCharges.val().toLowerCase()=="ib loan" || pCharges.val().toLowerCase()=="dbp loan" || pCharges.val().toLowerCase() == "tr loan" || pCharges.val().toLowerCase() == "dtr loan"){
    }else if(pCharges.val() == "IB_LOAN" || pCharges.val() == "DBP" || pCharges.val() == "EBP" || pCharges.val() == "TR_LOAN" || pCharges.val() == "DTR_LOAN" || pCharges.val() == "UA_LOAN" || pCharges.val() == "DUA_LOAN"){
        if ($("#accountNumber").length > 0) {
            $("#accountNumber").select2("disable");
        }
...
//        dNCharges2.val("");
        ...
        if (pCharges.val() == "DBP" || pCharges.val() == "EBP") {
...
//            $("input:radio[name=interestCodeFlagNegotiation]").filter('[value=M]').prop('disabled', 'disabled');
            computeNegotiationMaturityDate();
        } else if(pCharges.val() == "IB_LOAN" || pCharges.val() == 'TR_LOAN') {
            computeNegotiationMaturityDate();
        }else if(pCharges.val() == "UA_LOAN"){
...
function onPayClick(id) {
    ...
    if (formId == "#cashLcPaymentTabForm" || formId == "#productPaymentTabForm" || formId == "#paymentDetailsTabForm") {
        ...
        grid = $("#grid_list_charges_payment");
    } else if (formId == "#basicDetailsTabForm" && documentClass == "CDT") {
        grid = $("#grid_list_payment_cdt");
    }
...
//    alert(params.paymentDetailId+"\n"+params.supervisorId)
    var gridData = grid.jqGrid("getRowData", id);
    if (gridData.paymentMode in {IB_LOAN:1, TR_LOAN:1, UA_LOAN:1, DBP:1, EBP:1}) {
        ...
        params['agriAgraTagging'] = $("#agriAgra").val();
        params.push($.makeArray(gridData.setupString));
	} else if (gridData.paymentMode == 'AP'){
...
//        alert(JSON.stringify(data));
        if (data.success == true) {
            ...
            if (documentClass == "MD" || 
            	(documentClass == "EXPORT_ADVANCE" && referenceType == "DATA_ENTRY") || 
            	(documentClass == "IMPORT_CHARGES" && serviceType != "REFUND") || 
            	(documentClass == "IMPORT_ADVANCE") || 
            	(documentClass in {BP:1, CORRES_CHARGE:1, BC:1} && referenceType == "DATA_ENTRY")) {
...
function processLoan() {
    ...
    var params = {
        ...
        withCramApproval :   $("input[type=radio][name=cramApprovalFlag]:checked").val(),
...
function createLoan(){
    if(!validateRequiredFields('loan_details_popup','loanField')){
        triggerAlertMessage("Please fill out all the required fields hihi mode of payment ako.");
        return false;
    }
...
function updateGrid(id, action) {
    ...
    if(formId == "#chargesPaymentTabForm") {
        ...
        if(action == "pay") {
            ...
            if(gridData.paymentMode == "CASA") {
                payString = "<input type=\"button\" class=\"input_button_negative\" value=\"EC\" onclick=\"var id=\'"+id+"\'; onReversePaymentClick(id);\"/>"
            }else if(gridData.paymentMode == 'IB_LOAN' || gridData.paymentMode == 'TR_LOAN' || gridData.paymentMode == 'UA_LOAN' || gridData.paymentMode == 'DBP' || gridData.paymentMode == 'EBP'){
                payString = "<input type=\"button\" class=\"input_button\" value=\"Check\" onclick=\"var id=\'"+id+"\'; inquireLoanStatus(id);\"/>"
                status = "Processing"
            }
        } else if(action == "unpay"){
            if(gridData.paymentMode == 'IB_LOAN' || gridData.paymentMode == 'TR_LOAN' || gridData.paymentMode == 'UA_LOAN' || gridData.paymentMode == 'EBP'){
                payString = "<input type=\"button\" class=\"input_button\" value=\"Pay\" onclick=\"var id=\'"+id+"\'; showLoanDetails(id);\"/>"
            }else{
                payString = "<input type=\"button\" class=\"input_button\" value=\"Pay\" onclick=\"var id=\'"+id+"\'; onPayClick(id);\"/>"
            }
...
function onReversePaymentClick(id) {
    ...
    $.post(reversePaymentUrl, params, function (data) {
    	...
    	disablePopup("loading_div", "loading_bg");
        if (documentClass != "BP" && serviceType != "SETTLEMENT"){
        	grid.trigger("reloadGrid");
        } 
        if (documentClass == "BP" && serviceType == "NEGOTIATION") {
        	grid.trigger("reloadGrid");
        }
    });
}
...
function computeNegotiationMaturityDate() {
    ...
    if (loanTermNegotiation != "") {
        ...
        if(loanCodeFlagNegotiation == 'D'){
            ...
            if((typeof pCharges !== 'undefined' && pCharges.val() != "EBP") && loanTermNegotiation < 30) {
                $("#alertMessage").text("Loan term must be greater than 30.");
                triggerAlert();
...
function checkHoliday(data) {
	if (data["isHoliday"]) {
		...
		} else if ($("#modeOfPaymentCharges").val() == 'EBP' && data["isHoliday"] == false) {
			//var dateToAdjust = $.datepicker.parseDate('mm/dd/yy', $("#loanMaturityDate").val());
			//dateToAdjust.setDate(dateToAdjust.getDate() - 1);
			//$("#loanMaturityDate").val($.datepicker.formatDate('mm/dd/yy', dateToAdjust));
    ...
    $("#save_modeOfPaymentCharges").click(function(){
        ...
        case "CASA":
        	if($("#popup_modeOfPaymentCharges #accountName").val() == ""){
                ...
                return;
        	} else if(accountNumber && $("#accountNumber").val() != accountNumber){
        ...
        case "MD":
            ...
            break;
        case "EBP":
        	// Redmine 4122 - Validation #6d Holiday checking
        	// Set false when sibs has no data
//        	if ($("input[name=interestCodeFlagNegotiation]:checked").val() == undefined) {
//                errorCount ++;
//            }
//
//            if ($("input[name=loanCodeFlagNegotiation]:checked").val() == undefined) {
//                errorCount ++;
//            }

//            if (errorCount > 0) {
//                triggerAlertMessageUtils("Please fill out all the required fields.");
//            } else {
                var holidayUrl = checkIfHolidayUrl;
                var params = {
                    loanMaturityDate: $("#loanMaturityDate").val(),
...
//            }
        ...
        case "UA_LOAN":
            ...
            $(".display-ib-tr-loan :input").each (function() {
                if ($(this).attr("class") != undefined && $(this).attr("class") != null && $(this).attr("class").indexOf("popup_required") != -1) {
            ...
            if ($("input[name=cramApprovalFlag]:checked").val() == undefined) {
                errorCount ++;
            }
    ...
    $("#recomputeBalanceBtn").click(onRecomputeBalanceBtnClick);
    ...
    //for data entry ua loan settlement mt103 and pddts tabs
    /*if(serviceType.toLowerCase() == "ua loan settlement"){
     $(".mt103Tab_data_entry").hide();
     $(".pddtsTab_data_entry").hide();
     }*/

    //for data entry negotiation and ua loan settlement
    $(".credit_memo").hide();
//    $(".display_ibt_branch.").css("display","none");
    $("#md_balance").css("display", "none");
    ...
    $("#loanTermNegotiation,#repricingTermNegotiation,#interestTermNegotiation").blur(function () {
        ...
        if ($("input[name=" + codeFlag + "]:checked").val() == 'D' && $(this).attr("id") == "repricingTermNegotiation") {
            ...
            if($("#modeOfPaymentCharges").val() == "DBP" && parseInt($(this).val()) > 30){
    ...
    $("#close_loan_error_popup").live("click", function(){ disablePopup('loan_error_popup', 'view_loan_error_popup_bg'); });
...
//        if ($("#accountNumber").hasClass("input_field")) {
            var settlementCurrency = (formId == "#cashLcPaymentTabForm" || formId == "#productPaymentTabForm" || formId == "#paymentDetailsTabForm") ? $("#settlementCurrencyLc").val() : $("#settlementCurrency").val();
//            $.post(casaUserSearchUrl, {accountNumber: $("#accountNumber").val()}, function (data) {
            ...
            //alert("serviceType " + $("#serviceType").val())
            if ($("#serviceType").val() == "REFUND" ){
            	if($("#documentClass").val() == "IMPORT_ADVANCE" || $("#documentClass").val() == "EXPORT_ADVANCE"){
            		settlementCurrency = $("#newRefundCurrency").val();
            	}else if( $("#documentClass").val() != "CDT") {
            		settlementCurrency = $("#proceedsCurrencyModified").val();
            	} else if( $("#documentClass").val() != "CDT") {
            		settlementCurrency = $("#refundCurrency").val();
            	}
            ...
            if ($("#documentClass").val() == "CDT") {
                if ($("#serviceType").val() != "REFUND") {
                    if ($("#casaAccountNumber").length > 0) {
                        accountNumber = $("#casaAccountNumber").val();
...
//                        $("#accountNumber").val("");
//                    } else if (data['currency'] != settlementCurrency) {
...
//                    }
                    ...
                    // MARV: removed validation for account numbers if active or not
                    if (data['currency'] != settlementCurrency) {
                        triggerAlertMessage('Currency of account did not match Settlement Currency.');
                        $("#accountNumber").val("");
                    } else {
						accountNumber = $("#accountNumber").val().trim();
						$("#popup_modeOfPaymentCharges #accountNumber").val(data['accountNumber']);
                        $("#popup_modeOfPaymentCharges #accountName").val(data['accountName']);
