...
function constructChargesAndRatesPopup() {
    $.post(getBookedRatesUrl, {tradeServiceId: $("#transaction").val()}, function(data) {
        ...
        $.each(data.charges, function(key, charge) {
            ...
            //var newTextField = "<input type='text' id='" + charge.newId + "' class='input_field_medium_right' value='' />";
        ...
        $.each(data.corresCharges, function(key, corresCharges) {
            ...
            //var newTextField = "<input type='text' id='" + corresCharges.newId + "' class='input_field_medium_right' value='' />";
            var newTextField = "<input type='text' id='" + corresCharges.newId + "' class='input_field_right' name='number' onkeyup='if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')'/>";
...
function applyUpdatedCharges() {
    var params = {tradeServiceId: $("#tradeServiceId").val(), settlementCurrency: currency, transactionType: $("#transaction option:selected").text()};
    ...
    $.post(applyUpdatedChargesUrl, params, function(data) {
        ...
        $("#grid_refund_details_summary_for_charges").jqGrid('setGridParam', {url: savedNewChargesUrl, page: 1}).trigger("reloadGrid");
    });
}

function setTotalChargeRefundAmount() {
    var data = $("#grid_refund_details_summary_for_charges").getGridParam("userData");
