...
<script type="text/javascript">
    ...
    if(referenceType == "ETS") {
		if(serviceType.toUpperCase() == 'SETTLEMENT') {
	        saveUrl = '${g.createLink(controller: "drEtsSettlement", action: "saveSettlementEts")}';
	        updateUrl = '${g.createLink(controller: "drEtsSettlement", action: "updateSettlementEts")}';
	        updateStatusUrl = '${g.createLink(controller: "drEtsSettlement", action: "updateEtsStatus")}';
	        uploadDocumentUrl = '${g.createLink(controller: "drEtsSettlement", action: "uploadDocument")}';
            downloadDocumentUrl = '${g.createLink(controller:'drEtsSettlement', action:'downloadFile')}';
            deleteDocumentUrl = '${g.createLink(controller:'drEtsSettlement', action:'deleteDocument')}';
	    }
	    attachedDocumentsUrl = '${g.createLink(controller: "drEtsSettlement", action: "viewAttachments", params:[tradeServiceIdHolder:'xxxx'])}'.replace('xxxx',tradeServiceIdHolder);
	} else if(referenceType == "DATA_ENTRY") {
	    if(serviceType.toUpperCase() == "NEGOTIATION") {
	    	saveUrl = '${g.createLink(controller: "drDataEntryNegotiation", action: "saveNegotiationDataEntry")}';
	        updateUrl = '${g.createLink(controller: "drDataEntryNegotiation", action: "updateNegotiationDataEntry")}';
	        updateStatusUrl = '${g.createLink(controller: "drDataEntryNegotiation", action: "updateDataEntryStatus")}';
	    } else if(serviceType.toUpperCase() == "SETTLEMENT") {
	        saveUrl = '${g.createLink(controller: "drDataEntrySettlement", action: "saveSettlementDataEntry")}';
	        updateUrl = '${g.createLink(controller: "drDataEntrySettlement", action: "updateSettlementDataEntry")}';
	        updateStatusUrl = '${g.createLink(controller: "drDataEntrySettlement", action: "updateDataEntryStatus")}';
	        uploadDocumentUrl = '${g.createLink(controller: "drDataEntrySettlement", action: "uploadDocument")}';
            downloadDocumentUrl = '${g.createLink(controller:'drDataEntrySettlement', action:'downloadFile')}';
            deleteDocumentUrl = '${g.createLink(controller:'drDataEntrySettlement', action:'deleteDocument')}';
	        attachedDocumentsUrl = '${g.createLink(controller: "drDataEntrySettlement", action: "viewAttachments", params:[tradeServiceIdHolder:'xxxx'])}'.replace('xxxx',tradeServiceIdHolder);
        } else if(serviceType.toUpperCase() == "CANCELLATION") {
            saveUrl = '${g.createLink(controller: "drDataEntryCancellation", action: "saveCancellationDataEntry")}';
            updateUrl = '${g.createLink(controller: "drDataEntryCancellation", action: "updateCancellationDataEntry")}';
            updateStatusUrl = '${g.createLink(controller: "drDataEntryCancellation", action: "updateDataEntryStatus")}';
	    }
	} else if(referenceType == "PAYMENT") {
	    if(serviceType.toUpperCase() == 'SETTLEMENT') {
	    	gotoUrl = '${g.createLink(controller:'drChargesSettlement', action:'viewSettlementCharges')}';
	        updateUrl = '${g.createLink(controller:'drChargesSettlement', action:'updateSettlementCharges')}';
...
<div id="outer_wrap" <g:if test="${windowed}">class="window"</g:if> >
	...
	<g:if test="${referenceType.equalsIgnoreCase('eTS')}">
		...
		<g:render template="../nonlc/dr/dataentry/content" />
	</g:elseif>
	
	<g:if test="${referenceType.equalsIgnoreCase('PAYMENT')}">
