...
<script type="text/javascript">
    ...
    if(referenceType == "ETS") {
    	if(serviceType.toUpperCase() == 'SETTLEMENT') {
	        saveUrl = '${g.createLink(controller: "dpEtsSettlement", action: "saveSettlementEts")}';
	        updateUrl = '${g.createLink(controller: "dpEtsSettlement", action: "updateSettlementEts")}';
	        updateStatusUrl = '${g.createLink(controller: "dpEtsSettlement", action: "updateEtsStatus")}';
	        uploadDocumentUrl = '${g.createLink(controller: "dpEtsSettlement", action: "uploadDocument")}';
            downloadDocumentUrl = '${g.createLink(controller:'dpEtsSettlement', action:'downloadFile')}';
            deleteDocumentUrl = '${g.createLink(controller:'dpEtsSettlement', action:'deleteDocument')}';
        }
        attachedDocumentsUrl = '${g.createLink(controller: "dpEtsSettlement", action: "viewAttachments", params:[tradeServiceIdHolder:'xxxx'])}'.replace('xxxx',tradeServiceIdHolder);
    } else if(referenceType == "DATA_ENTRY") {
        if(serviceType.toUpperCase() == "NEGOTIATION") {
	    	saveUrl = '${g.createLink(controller: "dpDataEntryNegotiation", action: "saveNegotiationDataEntry")}';
	        updateUrl = '${g.createLink(controller: "dpDataEntryNegotiation", action: "updateNegotiationDataEntry")}';
	        updateStatusUrl = '${g.createLink(controller: "dpDataEntryNegotiation", action: "updateDataEntryStatus")}';
	    } else if((serviceType.toUpperCase() == "SETTLEMENT") || (serviceType.toUpperCase() == "SETTLEMENT_REVERSAL")) {
	        saveUrl = '${g.createLink(controller: "dpDataEntrySettlement", action: "saveSettlementDataEntry")}';
	        updateUrl = '${g.createLink(controller: "dpDataEntrySettlement", action: "updateSettlementDataEntry")}';
	        updateStatusUrl = '${g.createLink(controller: "dpDataEntrySettlement", action: "updateDataEntryStatus")}';
	        uploadDocumentUrl = '${g.createLink(controller: "dpDataEntrySettlement", action: "uploadDocument")}';
            downloadDocumentUrl = '${g.createLink(controller:'dpDataEntrySettlement', action:'downloadFile')}';
            deleteDocumentUrl = '${g.createLink(controller:'dpDataEntrySettlement', action:'deleteDocument')}';
	        attachedDocumentsUrl = '${g.createLink(controller: "dpDataEntrySettlement", action: "viewAttachments", params:[tradeServiceIdHolder:'xxxx'])}'.replace('xxxx',tradeServiceIdHolder);
        } else if(serviceType.toUpperCase() == "CANCELLATION") {
            saveUrl = '${g.createLink(controller: "dpDataEntryCancellation", action: "saveCancellationDataEntry")}';
            updateUrl = '${g.createLink(controller: "dpDataEntryCancellation", action: "updateCancellationDataEntry")}';
            updateStatusUrl = '${g.createLink(controller: "dpDataEntryCancellation", action: "updateDataEntryStatus")}';
	    }
    } else if(referenceType == "PAYMENT") {
        if(serviceType.toUpperCase() == 'SETTLEMENT') {
        	gotoUrl = '${g.createLink(controller:'dpChargesSettlement', action:'viewSettlementCharges')}';
            updateUrl = '${g.createLink(controller:'dpChargesSettlement', action:'updateSettlementCharges')}';
...
<div id="outer_wrap" <g:if test="${windowed}">class="window"</g:if> >
	...
	<g:elseif test="${referenceType.equalsIgnoreCase('DATA_ENTRY')}">
		<g:render template="../nonlc/dp/dataentry/content" />
	</g:elseif>
