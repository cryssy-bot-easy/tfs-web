...
<script type="text/javascript">
    ...
    if(referenceType == "ETS") {
    	if(serviceType.toUpperCase() == 'SETTLEMENT') {
	        saveUrl = '${g.createLink(controller: "oaEtsSettlement", action: "saveSettlementEts")}';
	        updateUrl = '${g.createLink(controller: "oaEtsSettlement", action: "updateSettlementEts")}';
	        updateStatusUrl = '${g.createLink(controller: "oaEtsSettlement", action: "updateEtsStatus")}';
	        uploadDocumentUrl = '${g.createLink(controller: "oaEtsSettlement", action: "uploadDocument")}';
            downloadDocumentUrl = '${g.createLink(controller:'oaEtsSettlement', action:'downloadFile')}';
            deleteDocumentUrl = '${g.createLink(controller:'oaEtsSettlement', action:'deleteDocument')}';
        }
        attachedDocumentsUrl = '${g.createLink(controller: "oaEtsSettlement", action: "viewAttachments", params:[tradeServiceIdHolder:'xxxx'])}'.replace('xxxx',tradeServiceIdHolder);
    } else if(referenceType == "DATA_ENTRY") {
        if(serviceType.toUpperCase() == "NEGOTIATION") {
	    	saveUrl = '${g.createLink(controller: "oaDataEntryNegotiation", action: "saveNegotiationDataEntry")}';
	        updateUrl = '${g.createLink(controller: "oaDataEntryNegotiation", action: "updateNegotiationDataEntry")}';
	        updateStatusUrl = '${g.createLink(controller: "oaDataEntryNegotiation", action: "updateDataEntryStatus")}';
	    } else if(serviceType.toUpperCase() == 'SETTLEMENT') {
	        saveUrl = '${g.createLink(controller: "oaDataEntrySettlement", action: "saveSettlementDataEntry")}';
	        updateUrl = '${g.createLink(controller: "oaDataEntrySettlement", action: "updateSettlementDataEntry")}';
	        updateStatusUrl = '${g.createLink(controller: "oaDataEntrySettlement", action: "updateDataEntryStatus")}';
            downloadDocumentUrl = '${g.createLink(controller:'oaDataEntrySettlement', action:'downloadFile')}';
            deleteDocumentUrl = '${g.createLink(controller:'oaDataEntrySettlement', action:'deleteDocument')}';
	        uploadDocumentUrl = '${g.createLink(controller: "oaDataEntrySettlement", action: "uploadDocument")}';
	        attachedDocumentsUrl = '${g.createLink(controller: "oaDataEntrySettlement", action: "viewAttachments", params:[tradeServiceIdHolder:'xxxx'])}'.replace('xxxx',tradeServiceIdHolder);
        } else if(serviceType.toUpperCase() == "CANCELLATION") {
            saveUrl = '${g.createLink(controller: "oaDataEntryCancellation", action: "saveCancellationDataEntry")}';
            updateUrl = '${g.createLink(controller: "oaDataEntryCancellation", action: "updateCancellationDataEntry")}';
            updateStatusUrl = '${g.createLink(controller: "oaDataEntryCancellation", action: "updateDataEntryStatus")}';
	    }
    } else if(referenceType == "PAYMENT") {
        if(serviceType.toUpperCase() == 'SETTLEMENT') {
        	gotoUrl = '${g.createLink(controller:'oaChargesSettlement', action:'viewSettlementCharges')}';
            updateUrl = '${g.createLink(controller:'oaChargesSettlement', action:'updateSettlementCharges')}';
...
<div id="outer_wrap" <g:if test="${windowed}">class="window"</g:if> >
	...
	<g:elseif test="${referenceType.equalsIgnoreCase('DATA_ENTRY')}">
		<g:render template="../nonlc/oa/dataentry/content" />
	</g:elseif>
