...
(revision)
	...
	Program [Revision] Details: 
		...
		2. Added save button after clicking Save/Accept in the popup of Mode of Settlement.
...
<g:hiddenField name="sampleId" value="${totalProceedsPayment}" />
...
<table class="tabs_forms_table">
    <tr>
        ...
        <td>
       		<input class="tags_currency select2_dropdown bigdrop settleCurrDd" name="newProceedsCurrency" id="newProceedsCurrency" data-orig="${newProceedsCurrency}" data-condition="${exportViaPddtsFlag == '1'}" />
       	</td>
    </tr>
</table>
...
<%--        <td class="field_label">Pass-on rates confirmed by: <g:if test="${exchange.size() > 1}"><span class="asterisk"> * </span></g:if></td>--%>
        <td class="field_label">Pass-on rates confirmed by:</td>
<%--        <td><g:textField name="passOnRateConfirmedBySettlement" value="${passOnRateConfirmedBySettlement ?: passOnRateConfirmedByCash}" data-orig="${passOnRateConfirmedBySettlement ?: passOnRateConfirmedByCash}" class="input_field ${exchange.size() > 1 ? 'required' : ''}"/></td>--%>
        <td><g:textField name="passOnRateConfirmedBySettlement" value="${passOnRateConfirmedByCash}" data-orig="${passOnRateConfirmedBySettlement ?: passOnRateConfirmedByCash}" class="input_field" readonly="readonly"/></td>
...
<g:if test="${((documentClass in["BP"]) && referenceType?.equals('DATA_ENTRY'))}">
    <table class="buttons_for_grid_wrapper saveButtonsContainer" hidden="true">
        <tr>
            <%-- BUTTON --%>
            <td><input type="button" id="saveConfirmSettlementToBen" class="input_button" style="margin-right: 41px;" value="Save" /></td>
        </tr>
        <tr>
            <%-- BUTTON --%>
            <td><input type="button" id="cancelConfirmSettlementToBen" class="input_button_negative" style="margin-right: 41px;" value="Cancel" /></td>
        </tr>
    </table>
    ...
    $(document).ready(function() {
    	...
    	if (${documentClass == 'BP' && documentType == 'FOREIGN' && serviceType == 'NEGOTIATION' && referenceType == 'DATA_ENTRY'}) {
	        $("#save_modeOfPaymentCharges").click(function() {
	    		if ($("#tabs").children(".ui-state-active").attr("aria-labelledby") === "proceedsDetailsTab") {
		    		console.log("testing kung pumasok sa save....");
		        	var productSummaryData = $("#grid_list_proceeds_payment_summary").jqGrid("getRowData");
					setTimeout(function() {
	        			$("#saveConfirmSettlementToBen").click();
					}, 1500);
	    		}
	        });

	        function validateCurrency() {
	        	var data = $("#grid_list_proceeds_payment_summary").jqGrid("getRowData");
	        	console.log($("#newProceedsCurrency").val());
				if ($("#newProceedsCurrency").val() !== $("#newProceedsCurrency").attr("data-orig") && data.length > 0) {
					triggerAlertMessage("Delete the Settlement Payment first");
					$("#newProceedsCurrency").setSettlementCurrencyDropdown($(this).attr("id")).select2('data',{id: $("#newProceedsCurrency").attr("data-orig")});
				}
	        }
...
<%--	  					usdToPhp: stripCommas($("#USD-PHP_special_rate_settlement").val() ? $("#USD-PHP_special_rate_settlement").val() : $("#USD-PHP_urr").val())--%>
        ...
        $("#recomputeChargeBtn").click(function() { 
