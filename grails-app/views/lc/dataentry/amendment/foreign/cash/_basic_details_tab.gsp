...
<g:if test="${session['group']?.equalsIgnoreCase("TSD")}" ></g:if>
<g:if test="${tsdInitiated}">
    ...
    <g:if test="${!tradeServiceReferenceNumber && !etsNumber}">
...
<g:hiddenField name="amountToFromEts" value="${amountTo}" />
...
<table class="tabs_forms_table">
	...
	<tr>
		...
		<td><g:textField class="input_field required" name="tinNumber" value="${tinNumber}"/> </td>
	...
	<tr>
		<td colspan="2"><span class="field_label" id="receiversReferenceLabel">Receiver's Reference Number</span></td>
		...
		<td><g:textField name="sendersReference" class="input_field" value="${documentNumber}" readonly="readonly"/></td>
	</tr>
	...
	<tr>
		...
		<td><input class="tags_cbcode select2_dropdown bigdrop" name="exporterCbCodeTo" id="exporterCbCodeTo" maxlength="60"/></td>
	</tr>
	<tr>
		<td>
	...
	<tr>
		...
		<td>
			<span class="field_label">
				<g:radio name="availableWithFlagTo" disabled="disabled" class="availableWithTo avialableWithB"  value="D" checked="${availableWithFlagTo == 'D' ? true : false}" disabled="true"/>
	...
	<tr>
		...
		<td><span class="field_label right_indent">To: <span class="clear_font">*</span></span></td>
	...
	<tr>
        ...
        <g:if test="${tsdInitiated!='true'}"><td><g:textField name="amountFrom" class="input_field_right numericCurrency" readonly="readonly" value="${amountFrom?:productAmount}"/></td></g:if>
        <g:else><td><g:textField name="amountFrom" class="input_field_right numericCurrency" readonly="readonly" value="${productAmount}"/>
        <g:hiddenField name="productAmount" value="${productAmount}"/></td></g:else>
	...
	<tr>
		...
		<td class="label_width"><span class="field_label"> Applicable Rules </span></td>
		<td class="input_width"><g:textField name="applicableRulesFrom" value="${applicableRulesFrom ?: applicableRules}" class="input_field" readonly="readonly"/></td>
		<td class="label_width"><span class="field_label right_indent">To: <span class="applicableRulesToAsterisk">*</span></span></td>
		<td class="input_width"><g:select name="applicableRulesTo" from="${['EUCP LATEST VERSION','EUCPURR LATEST VERSION', 'ISP LATEST VERSION', 'OTHR', 'UCP LATEST VERSION', 'UCPURR LATEST VERSION']}" noSelection="['':'SELECT ONE...']" keys="${['EUCP','EURR','ISP', 'OTHR', 'UCP', 'UCUR']}" value="${applicableRulesTo ?: 'UCP'}" class="select_dropdown" /></td>
	</tr>
	<tr class="other_rule_hide">
		<td> &#160; </td>
		<td>
			<span class="field_label ifOtherRule1 othr_label"> If 'OTHER': Other Rule </span>
		</td>
		
		<td>
			<g:textField name="otherRuleFrom" class="input_field ifOtherRuleFrom othr_field" value="${otherRuleFrom ?: otherRule}" readonly="readonly" />
		</td>
	...
	<tr class="advising_bank_hide">
		<td>
			<g:checkBox name="advisingBankSwitchDisplay" />
			<g:hiddenField name="advisingBankSwitch" value="${advisingBankSwitch ?: 'off'}" />
		</td>
		<td><span class="field_label advisingBank"> Advising Bank </span></td>
		<td>
			<g:textField name="confirmingBankFrom" value="${confirmingBankFrom ?: confirmingBank}" class="input_field advisingBankFrom" readonly="readonly" />
		</td>
	...
	<tr>
		<td>
			<g:checkBox name="expiryDateSwitchDisplay" />
		...
		<td><span class="field_label right_indent"><span class="expiryDateFlagLabel"></span>To: <span class="expiryDateToAsterisk">*</span></span></td>
		<td><g:textField name="expiryDateTo" value="${expiryDateTo}" class="datepicker_field" /></td>
	</tr>
	<tr>
		<td>
	...
	<tr>
		...
		<td>
			<tfs:formOfDocumentaryCredit name="formOfDocumentaryCreditFrom" disabled="true" class="select_dropdown" value="${formOfDocumentaryCreditFrom ?: formOfDocumentaryCredit}" />
		</td>
		<td><span class="field_label right_indent">To: <span class="formOfDocumentaryCreditToAsterisk">*</span></span></td>
		<td>
			<tfs:formOfDocumentaryCredit value="${formOfDocumentaryCreditTo}" name="formOfDocumentaryCreditTo" class="select_dropdown" />
		</td>
	</tr>
	<tr>
   		<td> &#160; </td>
...
</table> <%-- End of BASIC DETAILS Form--%>
<br /><br />
...
<g:if test="${tsdInitiated!='true'}">
    ...
    function setupLcAmountFlagLabel() {
        if($("#amountTo").val().length != 0) {
            if($("#lcAmountFlag").val() == 'INC') {
                $(".lcAmountFlagLabel").text("Increase").addClass("field_label");
            } else if($("#lcAmountFlag").val() == 'DEC') {
                $(".lcAmountFlagLabel").text("Decrease").addClass("field_label");
            } else {
                $(".lcAmountFlagLabel").text("");
            }
        }
    }
...
function setupExpiryDateFlagLabel() {
    if($("#expiryDateTo").val().length != 0) {
        if($("#expiryDateFlag").val() == 'EXT') {
            $(".expiryDateFlagLabel").text("Extend").addClass("field_label");
        } else if($("#lcAmountFlag").val() == 'RED') {
            $(".expiryDateFlagLabel").text("Reduce").addClass("field_label");
        } else {
            $(".expiryDateFlagLabel").text("");
        }
    }
}
...
function onChangeConfirmationInstruction(toggledConfirmation) {
	...
	if(confirmationInstructionsFlagTo == "YES" || confirmationInstructionsFlagTo == "MAY ADD") {
		...
		if (toggledConfirmation) {
			onChangeConfirmationInstruction$("#availableByTo").val("P");
		}
		$(".showDrawee").show();
...
$(document).ready(function() {
	...
	if(tsdInitiated) {
        ...
        setupLcAmountFlagLabel();
    }
	

	setupExpiryDateFlagLabel();
