...
class EncodeClientDetailsController {
	...
	def viewCdt() {
		...
		if(session.otherImportsModel) {
			...
			render(view: "/others/imports/cdt/encode_client_details/content", model: chainModel ? chainModel : session.otherImportsModel)
		}else {
	...
	def viewEncodeClientDetailsCdt() {
