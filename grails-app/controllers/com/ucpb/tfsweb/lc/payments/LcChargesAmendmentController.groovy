...
class LcChargesAmendmentController {
...
//            }
    ...
    def viewAmendmentCharges() {
        ...
        if (params.tradeServiceId) {
            ...
            dataEntryMap.each {
                if (!it.key.equals("referenceType") && !it.key.equals("serviceType") && !it.key.equals("documentClass")) {
                    if (it.key.equals("outstandingBalance")) {
                        if (new BigDecimal(it.value) < 0) {
                            session.chargesModel << [outstandingBalance: BigDecimal.ZERO]
                        } else {
        ...
        chain(action: "viewAmendment", model: session.chargesModel)
    }

    def updateAmendmentCharges() {
        //construct header title
