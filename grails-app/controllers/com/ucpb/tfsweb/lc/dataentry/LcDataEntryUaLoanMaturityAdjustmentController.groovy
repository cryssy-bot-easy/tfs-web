...
class LcDataEntryUaLoanMaturityAdjustmentController {
	...
	def viewMaturityAdjustmentDataEntry() {
        ...
        def documentServiceRoute = routingInformationService.getNextMainApprover(DOCUMENT_CLASS, documentType, documentSubType1, documentSubType2, REFERENCE_TYPE.toUpperCase(), "UA_LOAN_MATURITY_ADJUSTMENT", session.username, session.userrole.id, session.unitcode, session.dataEntryModel, session.userLevel)
        session.nextRoute = documentServiceRoute
        session.dataEntryModel << routingInformationService.getMainApprovalMode(DOCUMENT_CLASS, documentType, documentSubType1, documentSubType2, "UA_LOAN_MATURITY_ADJUSTMENT", session.dataEntryModel.approvers)
        ...
        chain(action:"viewMaturityAdjustment", model: session.dataEntryModel)
	}
	
	def updateMaturityAdjustmentDataEntry() {
        // get lc class and type
        String documentType = params.documentType
        String documentSubType1 = params.documentSubType1.toUpperCase()
        String documentSubType2 = params.documentSubType2
    ...
    def uploadDocument() {
        ...
        redirect(controller:'lcDataEntryUaLoanMaturityAdjustment', action:'invokeUploadCommand',params:params)
    }
