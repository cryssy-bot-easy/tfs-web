...
class LcDataEntryIndemnityCancellationController {
	...
	def viewIndemnityCancellationDataEntry() {
        String headerTitle
        ...
        if (params.tradeServiceId) {
            ...
            headerTitle = headerService.getDataEntryTitle(documentType, DOCUMENT_CLASS, documentSubType1, SERVICE_TYPE, documentSubType2)
        ...
        def documentServiceRoute = routingInformationService.getNextMainApproverTsdInitiated(DOCUMENT_CLASS, params.documentType, documentSubType1, documentSubType2, REFERENCE_TYPE.toUpperCase(), SERVICE_TYPE?.toUpperCase(), session.username, session.userrole.id, session.unitcode, session.dataEntryModel, session.userLevel)
        ...
        chain(action:"viewIndemnityCancellation", model: session.dataEntryModel)
	}

	def saveIndemnityCancellationDataEntry() {
        String headerTitle = headerService.getDataEntryTitle(params.documentType, DOCUMENT_CLASS, params.documentSubType1, SERVICE_TYPE, params.documentSubType2)

        session.dataEntryModel = [documentClass: DOCUMENT_CLASS, documentType: params.documentType, title: headerTitle, serviceType: SERVICE_TYPE, referenceType: REFERENCE_TYPE]
	...
	def updateIndemnityCancellationDataEntry() {
        // get lc class and type
        String documentType = params.documentType
        String documentSubType1 = params.documentSubType1.toUpperCase()
        String documentSubType2 = params.documentSubType2
        ...
        params.put("chargesOverridenFlag", "N")
        params.put("username", session.username)
        params.put("unitcode", session.unitcode)
        // trigger command
    ...
    def uploadDocument() {
        ...
        redirect(controller:'lcDataEntryIndemnityCancellation', action:'invokeUploadCommand',params:params)
    }
