 ...
 *	Member Type: groovy
 *	Project: WEB
 *	Project Name: LcEtsNegotiationController.groovy
*/
/**
 * SCR/ER Description:
 *	[Revised by:] Cedrick C. Nungay
 *	[Date revised:] 03/21/2018
 *	Program [Revision] Details: Added initial value on UI for documentaryStampForFirstAmountPopup and documentaryStampAmountForNextAmountPopup.
 *	Date deployment: 3/20/2018
 *	Member Type: groovy
 *	Project: WEB
 *	Project Name: LcEtsNegotiationController.groovy
 */
class LcEtsNegotiationController {
     ...
     *  Testing Purposes
     */
    int OVERDRAWN_NEGOTIATION_AMOUNT = 1 // set to 0 or any positive value, if not 0 , Field Overdrawn Negotiation Amount appears ~~ NEGOTIATION PAYMENT TAB

    // render page for viewing
    def viewNegotiation() {
        ////println"viewNegotiation"
        // if accessed from create transaction

        if (chainModel) {
            session.etsModel = chainModel

//            ////println"serviceInstructionId: " + session.etsModel.serviceInstructionId
...
//				session.etsModel << [exchange:exchange]
                ...
                println "chainModel.negotiationCurrency chainModel.negotiationCurrency chainModel.negotiationCurrency:" + chainModel.negotiationCurrency
...
//            def chainModel = [:]
//            chainModel.currency = session.etsModel?.currency
...
//            session.etsModel << [exchange:exchange]
        ...
        String lcNumber = session.etsModel?.lcNumber ?: session.etsModel?.documentNumber
        println "lcNumberlcNumberlcNumberlcNumberlcNumberlcNumber > " + lcNumber
        List<Map<String, Object>> icNumbers = documentClassService.findAllIcNumbers(lcNumber)
        session.etsModel << [icNumbers: icNumbers?.ICNUMBER]
		
		println "LcEtsNegotiationController.viewNegotiation (line 180)"
		BigDecimal totalIcAmount = BigDecimal.ZERO
		BigDecimal totalIcCashAmount = BigDecimal.ZERO
		
		println "Before totalIcAmount : " + session?.etsModel?.totalIcAmount
		if (session?.etsModel?.totalIcAmount == null) {					
			totalIcAmount = documentClassService.findTotalIcAmount(lcNumber).get("TOTALICAMOUNT")			
			totalIcCashAmount = documentClassService.findTotalIcAmount(lcNumber).get("TOTALICCASHAMOUNT")
			session.etsModel << [totalIcAmount: totalIcAmount]
			session.etsModel << [totalIcCashAmount: totalIcCashAmount]
		}		
		println "After totalIcAmount : " + session.etsModel.totalIcAmount
		
		println "Before outstandingBalanceLessIc : " + session?.etsModel?.outstandingBalanceLessIc
		if (session?.etsModel?.outstandingBalanceLessIc == null) {
			def outstandingBalanceLessIc = (session.etsModel.outstandingBalance ?: 0) - totalIcCashAmount	
			outstandingBalanceLessIc = (outstandingBalanceLessIc>0) ? outstandingBalanceLessIc : 0
			session.etsModel << [outstandingBalanceLessIc: outstandingBalanceLessIc]
		}	
		println "After outstandingBalanceLessIc : " + session.etsModel.outstandingBalanceLessIc
				
		if (session.etsModel.senderToReceiver != null && 
			!session.etsModel.senderToReceiver.equalsIgnoreCase("")){
			session.etsModel << [senderToReceiver: ""]
		}	
				
		if (session.etsModel.senderToReceiverInformation != null &&
			!session.etsModel.senderToReceiverInformation.equalsIgnoreCase("")){
			session.etsModel << [senderToReceiverInformation: ""]
		}
	
		if (session.etsModel.senderToReceiverInformationNarrative != null &&
			!session.etsModel.senderToReceiverInformationNarrative.equalsIgnoreCase("")){
			session.etsModel << [senderToReceiverInformationNarrative: ""]
		}
        ...
        if (session.cancelBd) {
			...
			def cp = coreAPIService.dummySendQuery(
				[documentType: session.etsModel.documentType, documentClass: session.etsModel.documentClass,
					...
					serviceType: 'NEGOTIATION'], 'getChargesParameter', 'chargesParameter/').result
...
//        session.etsModel << [exchange:exchange]
        ...
        session.etsModel.totalAmountDueLc = session.etsModel.negotiationAmount
        session.etsModel.amount = session.etsModel.negotiationAmount
        session.etsModel << [viewMode: params.viewMode]
        // chain to render page
        chain(action: "viewNegotiation", model: session.etsModel)
    }
    ...
    def uploadDocument() {
        ...
        redirect(controller: 'lcEtsNegotiation', action: 'invokeUploadCommand', params: params)
    }
    ...
    def addRemarks() {
        // trigger command
//		commandService.updateCommand(params)
