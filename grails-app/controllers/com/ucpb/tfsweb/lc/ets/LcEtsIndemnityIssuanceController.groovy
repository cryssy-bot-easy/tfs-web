...
class LcEtsIndemnityIssuanceController {
    ...
    def viewIndemnityIssuance() {
        ...
        if(chainModel) {
            ...
            println "angol angol indemnity"

            if(chainModel.shipmentCurrency) {
                println "angol angol indemnity"
                def exchange = []
                ...
                if (session.etsModel.documentSubType1.equals("REGULAR") || session.etsModel.documentSubType1.equals("STANDBY")) {
                    ratesResult = ratesService.getRegularSellRates(chainModel.shipmentCurrency).response.details
                } else if (session.etsModel.documentSubType1.equals("CASH")) {
                    ratesResult = ratesService.getCashSellRates(chainModel.shipmentCurrency).response.details
                }
...
//            session.etsModel << [accountsReceivable: parserService.listHashMapToString(accountsReceivable)]
	...
	def viewIndemnityIssuanceEts() {
        ...
        if (params.tradeServiceId) {
            ...
            ////println"accessed from unacted screen ets..."
			...
			println "etsApprovers: " + session.etsModel.approvers
        ...
        chain(action:"viewIndemnityIssuance", model: session.etsModel)
	}
	
	// saves new instance of ets
	def saveIndemnityIssuanceEts() {
    ...
    def updateIndemnityIssuanceEts() {
        String documentType = params.documentType
        String documentSubType1 = params.documentSubType1.toUpperCase()
        String documentSubType2 = params.documentSubType2
    ...
    def uploadDocument() {
        ...
        redirect(controller:'lcEtsIndemnityIssuance', action:'invokeUploadCommand',params:params)
    }
