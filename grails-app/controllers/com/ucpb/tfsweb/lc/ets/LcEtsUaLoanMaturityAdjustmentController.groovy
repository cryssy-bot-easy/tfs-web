...
class LcEtsUaLoanMaturityAdjustmentController {
    ...
    protected String SERVICE_TYPE = "UA Loan Maturity Adjustment"
    protected String DOCUMENT_CLASS = "LC"
	
	// render page for viewing
    def viewMaturityAdjustment() {
		if(chainModel) {
...
//            session.etsModel << [accountsReceivable: parserService.listHashMapToString(accountsReceivable)]
            ...
            println "chainModel.negotiationCurrency:"+chainModel.negotiationCurrency
            println "chainModel.currency:"+chainModel.currency
            String currencyUsed = chainModel.currency ?: chainModel.negotiationCurrency
            println "currencyUsed:"+currencyUsed
            if (session.etsModel.documentType.equals("DOMESTIC")){
                ...
                if (session.etsModel.documentSubType1.equals("REGULAR") || session.etsModel.documentSubType1.equals("STANDBY")) {
                    ratesResult = ratesService.getRegularSellRatesDM(currencyUsed).response.details
                } else if (session.etsModel.documentSubType1.equals("CASH")) {
                    ratesResult = ratesService.getCashSellRatesDM(currencyUsed).response.details
                }
            } else {
                if (session.etsModel.documentSubType1.equals("REGULAR") || session.etsModel.documentSubType1.equals("STANDBY")) {
                    ratesResult = ratesService.getRegularSellRates(currencyUsed).response.details
                } else if (session.etsModel.documentSubType1.equals("CASH")) {
                    ratesResult = ratesService.getCashSellRates(currencyUsed).response.details
                }
            }
	...
	def viewMaturityAdjustmentEts() {
        ...
        if (params.tradeServiceId) {
            ...
            def documentServiceRoute = routingInformationService.getNextBranchApprover(DOCUMENT_CLASS, documentType, documentSubType1, documentSubType2, REFERENCE_TYPE.toUpperCase(), "UA_LOAN_MATURITY_ADJUSTMENT", session.username, session.userrole.id, session.unitcode, session.etsModel)
            session.nextRoute = documentServiceRoute
            session.etsModel << routingInformationService.getBranchApprovalMode(DOCUMENT_CLASS, documentType, documentSubType1, documentSubType2, "UA_LOAN_MATURITY_ADJUSTMENT", session.etsModel.approvers)

        }
		session.etsModel<<[viewMode:params.viewMode]
        // chain to render page
        chain(action:"viewMaturityAdjustment", model: session.etsModel)
	}
	
	// saves new instance of ets
	def saveMaturityAdjustmentEts() {
        String documentType = params.documentType
        String documentSubType1 = params.documentSubType1.toUpperCase()
        String documentSubType2 = params.documentSubType2
...
//        session.etsModel << [nextRoute: documentServiceRoute]
//        session.etsModel << routingInformationService.getBranchApprovalMode(DOCUMENT_CLASS, documentType, documentSubType1, documentSubType2,SERVICE_TYPE?.toUpperCase(), session.etsModel.approvers)
	...
	def updateMaturityAdjustmentEts() {
        String documentType = params.documentType
        String documentSubType1 = params.documentSubType1.toUpperCase()
        String documentSubType2 = params.documentSubType2
...
//        session.etsModel << routingInformationService.getBranchApprovalMode(DOCUMENT_CLASS, documentType, documentSubType1, documentSubType2,SERVICE_TYPE?.toUpperCase(), session.etsModel.approvers)
    ...
    def uploadDocument() {
        ...
        redirect(controller:'lcEtsUaLoanMaturityAdjustment', action:'invokeUploadCommand',params:params)
    }
