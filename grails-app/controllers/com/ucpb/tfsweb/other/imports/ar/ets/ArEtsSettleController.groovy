package com.ucpb.tfsweb.other.imports.ar.ets

import grails.converters.JSON
import org.springframework.web.multipart.MultipartHttpServletRequest;
import org.springframework.web.multipart.MultipartFile;

class ArEtsSettleController {
    ...
    protected String SERVICE_TYPE = "SETTLE"
    ...
    def viewSettle() {
        ...
        if(session.etsModel) {
            ...
            render(view: "/others/imports/ar/index", model: chainModel ?: session.etsModel)
        }else {
    ...
    def viewSettleEts() {
        ...
        chain(action:"viewSettle", model: session.etsModel)
    }

    def saveSettleEts() {
        //construct header title
        String headerTitle = "AR Monitoring Settlement - eTS"
        ...
        etsMap.each{
			...
			if((it.key.equals("originalAmount") || it.key.equals("amount")) && !session.etsModel?.amountDue){
				session.etsModel << [amountDue: it.value]
			}
        }
		def documentServiceRoute = routingInformationService.getNextBranchApprover(DOCUMENT_CLASS, null, null, null, REFERENCE_TYPE.toUpperCase(), SERVICE_TYPE?.toUpperCase(), session.username, session.userrole.id, session.unitcode, session.etsModel)
		session.nextRoute = documentServiceRoute
    ...
    def updateEtsStatus() {
        //construct header title
        String headerTitle = "AR Monitoring Settle - eTS"
	...
	def uploadDocument() {
		...
		redirect(controller:'arEtsSettle', action:'invokeUploadCommand',params:params)
	}
