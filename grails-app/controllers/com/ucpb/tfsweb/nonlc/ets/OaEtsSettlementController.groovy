 ...
 *	Member Type: groovy
 *	Project: WEB
 *	Project Name: OaEtsSettlementController.groovy
*/
/**
 * PROLOGUE
 * SCR/ER Description:
 ...
 *	Project Name: OaEtsSettlementController.groovy
*/
class OaEtsSettlementController {
...
//        if (session.etsModel.settleFlag && "Y".equals(session.etsModel.settleFlag)) {
		...
		if(session.cancelBd){
            ...
            render(view: "/nonlc/oa/index", model: chainModel ? chainModel : session.etsModel)
        }else {
...
//		String headerTitle = headerService.getEtsTitle(DOCUMENT_TYPE, DOCUMENT_CLASS, null, SERVICE_TYPE + (reverseEts ? " Reversal" : ""))
        ...
        println "session.etsModel.reverseEts =============" + session.etsModel.reverseEts
