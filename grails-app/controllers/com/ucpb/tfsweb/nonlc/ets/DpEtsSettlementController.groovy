 ...
 *	Member Type: groovy
 *	Project: WEB
 *	Project Name: DpEtsSettlementController.groovy
*/
/**
 * PROLOGUE
 * SCR/ER Description:
 ...
 *	Project Name: DpEtsSettlementController.groovy
*/
class DpEtsSettlementController {
    ...
    def viewSettlement() {
		session.etsModel << [hasCif : documentClassService.getDocumentaryCredit(session.etsModel.documentNumber, DOCUMENT_CLASS, "NEGOTIATION").cifNumber]
...
//            session.etsModel << [approvalMode: 'APPROVE']
        if ("FOREIGN".equals(session.etsModel.documentType)) {
            if (session.etsModel.settlementMode && !(session.etsModel.settlementMode.equals("TR"))) {
                session.etsModel << [approvalMode: 'APPROVE']
            } else {
                session.etsModel << routingInformationService.getBranchApprovalMode(DOCUMENT_CLASS, session.etsModel.documentType, "", "",SERVICE_TYPE?.toUpperCase(), session.etsModel.approvers)
            }
        } else {
		...
		if(session.cancelBd){
            ...
            render(view: "/nonlc/dp/index", model: chainModel ? chainModel : session.etsModel)
        }else {
...
//		String headerTitle = headerService.getEtsTitle(documentType, DOCUMENT_CLASS, null, SERVICE_TYPE + (reverseEts ? " Reversal" : ""))
		String headerTitle = reverseEts ? headerService.getNonLcReversalTitle(documentType, DOCUMENT_CLASS, SERVICE_TYPE, REFERENCE_TYPE) : headerService.getEtsTitle(documentType, DOCUMENT_CLASS, "", SERVICE_TYPE, "")

        session.etsModel = [documentType: documentType,  documentClass: DOCUMENT_CLASS, title: headerTitle, serviceType: SERVICE_TYPE, referenceType: REFERENCE_TYPE, windowed: windowed]
        ...
        if (params.etsNumber) {
			...
			def documentServiceRoute = routingInformationService.getNextBranchApprover(DOCUMENT_CLASS, documentType, "", "", REFERENCE_TYPE.toUpperCase(), SERVICE_TYPE?.toUpperCase(), session.username, session.userrole.id, session.unitcode, session.etsModel)
			session.nextRoute = documentServiceRoute

//			session.etsModel << routingInformationService.getBranchApprovalMode(DOCUMENT_CLASS, documentType, "", "",SERVICE_TYPE?.toUpperCase(), session.etsModel.approvers)
//            if (session.etsModel.settlementMode && !(session.etsModel.settlementMode.equals("TR") || session.etsModel.settlementMode.equals("DTR"))) {
            if (session.etsModel.settleFlag && "Y".equals(session.etsModel.settleFlag) && "DOMESTIC".equals(session.etsModel.documentType)) {
                session.etsModel << [approvalMode: 'APPROVE']
            } else {
                session.etsModel << routingInformationService.getBranchApprovalMode(DOCUMENT_CLASS, documentType, "", "",SERVICE_TYPE?.toUpperCase(), session.etsModel.approvers)
            }
        ...
        println session.etsModel.approvalMode
    ...
    def saveSettlementEts() {
        ...
        String headerTitle = headerService.getEtsTitle(documentType, DOCUMENT_CLASS, "", SERVICE_TYPE, "")

        session.etsModel = [documentType: documentType,  documentClass: DOCUMENT_CLASS, title: headerTitle, serviceType: SERVICE_TYPE, referenceType: REFERENCE_TYPE]
