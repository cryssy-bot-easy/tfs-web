...
class DaDataEntryNegotiationAcceptanceController {
    ...
    protected String SERVICE_TYPE = "Negotiation Acceptance"
    protected String DOCUMENT_CLASS = "DA"
	protected String DOCUMENT_TYPE = "FOREIGN"

    // render page
    def viewNegotiationAcceptance() {
        if(chainModel) {
    ...
    def viewNegotiationAcceptanceDataEntry() {
//        String headerTitle = "FX Document Against Acceptance Negotiation Acceptance Non-LC - Data Entry"
...
//			Map<String, Object> dataEntryMap = dataEntryService.getDataEntry(params.tradeServiceId)
        ...
        def documentServiceRoute = routingInformationService.getNextMainApprover(DOCUMENT_CLASS, DOCUMENT_TYPE, "", "", REFERENCE_TYPE.toUpperCase(), "NEGOTIATION_ACCEPTANCE", session.username, session.userrole.id, session.unitcode, session.dataEntryModel, session.userLevel)
        session.nextRoute = documentServiceRoute
        session.dataEntryModel << routingInformationService.getMainApprovalMode(DOCUMENT_CLASS, DOCUMENT_TYPE, "", "","NEGOTIATION_ACCEPTANCE", session.dataEntryModel.approvers)
        ...
        def productReference = routingInformationService.getProductReferences(DOCUMENT_CLASS, DOCUMENT_TYPE, "", "", "NEGOTIATION_ACCEPTANCE", session.dataEntryModel, session.unitcode, session.username)
        ...
        chain(action: "viewNegotiationAcceptance", model: session.dataEntryModel)
    }

    def saveNegotiationAcceptanceDataEntry() {
    ...
    def updateNegotiationAcceptanceDataEntry() {
