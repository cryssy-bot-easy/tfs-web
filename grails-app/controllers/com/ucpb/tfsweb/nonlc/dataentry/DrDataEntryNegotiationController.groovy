...
class DrDataEntryNegotiationController {
    ...
    def viewNegotiation() {
		...
		if(session.cancelBd){
            ...
            def urrrates = foreignExchangeService.formatUrrRates(ratesService.getRatesByBaseCurrency().display, chainModel)
            //println"urrrates:"+urrrates
            session.dataEntryModel << [urrrates:urrrates]

            session.dataEntryModel << [tsdInitiated:"true"]

            render(view: "/nonlc/dr/index", model:chainModel ? chainModel : session.dataEntryModel)
        }else{
