...
class DaChargesSettlementController {
...
//                session.chargesModel << [cilexFlag: cilexFlag]
        ...
        if(session.chargesModel) {
            Map<String, Object> paymentsMade = chargesPaymentService.findAllPayments(session.chargesModel?.tradeServiceId)
            session.chargesModel << [paymentsMade: paymentsMade.payments]

            render(view: "/nonlc/da/index", model:chainModel ? chainModel : session.chargesModel)
        }else{
    ...
    def viewSettlementCharges() {
		...
		String headerTitle = headerService.getChargesTitle(DOCUMENT_TYPE, DOCUMENT_CLASS, "", SERVICE_TYPE + (reverseDE ? " Reversal" : ""), "")
		
        String reversalDENumber = ""
        String reversedDENumber = ""
        String reversalEtsNumber = ""
        String approvers=""
        session.chargesModel = [documentType: DOCUMENT_TYPE,  documentClass: DOCUMENT_CLASS, title: headerTitle, serviceType: SERVICE_TYPE, referenceType: REFERENCE_TYPE]
...
//                session.chargesModel << [cilexFlag: "true"]
    ...
    def updateSettlementCharges() {
		String headerTitle = headerService.getChargesTitle(DOCUMENT_TYPE, DOCUMENT_CLASS, "", SERVICE_TYPE, "")
